#!/usr/bin/env python3
"""Apply the corrected final Retail 12.1 runtime hardening pass."""
...
''',
    ...
    refresh_function = '''function addon:RefreshTooltipSafe(tooltip, reason)
...
def patch_modifier_refresh() -> None:
    function_text = '''local function HasRefreshableIDContext(context)
...
def patch_style_hooks() -> None:
    function_text = '''local function InstallTooltipHooks(tooltip)
    ...
    -- Registration during a temporary ScriptBindings restriction remains
    -- retryable. EnsureStyle() will install hooks once access returns.
...
def patch_mythic_plus_and_inspect() -> None:
    cache_block = '''local mythicPlusCache = {}
...
''',
        '''    -- NotifyInspect is one global request channel. Throttle all targets,
...
'''
    replace_once(
        ...
        listener + "RegisterKnownTooltips(false)\n",
    ...
    if attach_call in text:
        ...
        block_end_marker = "\n    end"
        block_end = text.index(block_end_marker, call_pos) + len(block_end_marker)
...
def patch_docs() -> None:
    ...
    old = '''- Added a bounded 60-second Mythic+ summary cache to avoid duplicate API work on
  repeated unit hovers.
'''
    new = '''- Added a bounded 60-second Mythic+ summary cache to avoid duplicate API work on
...
'''
    ...
    old = '''Inspect requests are asynchronous, throttled, and cached. Mythic+ summaries are
bounded and cached briefly. Mount journal data and saved raid progress are
collected outside repeated tooltip work.
'''
    new = '''Inspect requests are asynchronous, globally throttled, and cached. Mythic+
...
def main() -> None:
    ...
    patch_style_hooks()
    patch_mythic_plus_and_inspect()
    patch_item_cache_refresh()
    patch_docs()
    print("Corrected final Retail 12.1 hardening applied.")
